...
import controleur.C_VSql_Vue_TotalBilletEleve;
import controleur.Tableau;
import controleur.VSql_Vue_TotalBilletEleve;

public class P_VueTotalRet extends P_Principal implements ActionListener {
	...
	public P_VueTotalRet () {
		...
		this.cbxFiltre.addItem("nom_prenom");
		this.cbxFiltre.addItem("nbBillets");
		this.cbxFiltre.addItem("dureeCumulee");
		...
		String entetes [] = {"ID Etudiant", "Nom Prenom", "nb Billets", "duree cumulee"};
	...
	public Object[][] obtenirDonnees(){
		ArrayList<VSql_Vue_TotalBilletEleve> lesVueLPts = C_VSql_Vue_TotalBilletEleve.selectAllVues();
		Object[][] matrice = new Object [lesVueLPts.size()][4];
		int i=0;
		for (VSql_Vue_TotalBilletEleve uneVueLPt : lesVueLPts) {
			matrice[i][0] = uneVueLPt.getIdE();
			matrice[i][1] = uneVueLPt.getNomprenom();
			matrice[i][2] = uneVueLPt.getNbBillets();
			matrice[i][3] = uneVueLPt.getDureeCumulee();
			i++;
		}
		return matrice;
	}
	public Object[][] obtenirDonnees(ArrayList<VSql_Vue_TotalBilletEleve> lesVues){
		Object[][]matrice = new Object[lesVues.size()][4];
		int i = 0;
		for (VSql_Vue_TotalBilletEleve uneVueLPt : lesVues) {
			matrice[i][0] = uneVueLPt.getIdE();
			matrice[i][1] = uneVueLPt.getNomprenom();
			matrice[i][2] = uneVueLPt.getNbBillets();
			matrice[i][3] = uneVueLPt.getDureeCumulee();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.btAnnuler) {
			...
			ArrayList<VSql_Vue_TotalBilletEleve> lesVues = C_VSql_Vue_TotalBilletEleve.selectSearch(attribut,mot);
